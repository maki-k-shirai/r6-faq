...
export default async function SearchPage({
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 space-y-6">
      ...
      {/* ゼロヒット */}
      {query && totalCount === 0 && (
        <div className="rounded-md border bg-white px-5 py-8 text-center shadow-sm ring-1 ring-black/5">
          <p className="text-sm text-gray-500">
            「{query}」に一致するコンテンツは見つかりませんでした。
          </p>
          <p className="mt-1 text-xs text-gray-400">別のキーワードをお試しください。</p>
          <div className="mt-4 flex flex-wrap justify-center gap-2">
            {SUGGESTED_KEYWORDS.map((kw) => (
              <Link
                key={kw}
                href={`/search?q=${encodeURIComponent(kw)}`}
                className="rounded-full border border-gray-200 bg-white px-3 py-0.5 text-xs text-gray-600 transition hover:border-gray-400 hover:bg-gray-50"
              >
                {kw}
              </Link>
            ))}
              ...
              {items.map((item) => (
                <li key={`${item.section}-${item.slug}`}>
                  ...
                  >
                    ...
                    {item.excerpt && (
                      <p className="mt-0.5 truncate text-xs text-gray-500">{item.excerpt}</p>
                    )}
                    {item.updated_at && (
                      <p className="mt-1 text-xs text-gray-400">{item.updated_at}</p>
                    )}
                  </Link>
                </li>
              ))}
      ...
      {/* クエリなし（直接アクセス） */}
      {!query && (
        <p className="text-sm text-gray-400">上の検索欄からキーワードを入力してください。</p>
      )}
